...
def train(device, data, word_count, mode, vocabulary_size, embedding_dim, batch_size,
    ...
    for i in tqdm(xrange(num_steps)):
        ...
        loss_val += loss.item()
        if i > 0 and (i+1) % int(num_steps/10) == 0:
            print(' ',data_index,'/',len(data), time.time()-t,'s')
            t = time.time()
            print('Average loss at step', i, ':', loss_val/(num_steps/10))
            loss_val = 0
    
    return model.get_embeddings(), model
...
def check_targets_included(target_path, dictionary):
    lines = []
    with open(target_path,mode='r') as f:
        lines = f.readlines()
    # words = []
...
if __name__ == '__main__':
    ...
    final_embeddings,model = train(device=dev,
                             data=data,
    ...
    torch.save(model.state_dict(), 'model_'+args.output)
